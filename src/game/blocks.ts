...
export class Registry {
  ...
  private registerBlocks() {
    ...
    this.blk({ id: B.GLASS, name: 'glass', displayName: 'Glass', category: 'building', textures: tex('glass'), hardness: 0.3, transparent: true, opaque: false, drops: [] });
  ...
  private registerItems() {
    ...
    const tiers: [ItemId, ItemId, string, number, number, number][] = [
      // [toolBaseId, name..., tier, durability, miningSpeed, attackDamage]
    ...
    for (const [pickId, , mat, tier, dur, speed, dmg] of tiers) {
      const names: Record<string, string> = { wood: 'Wood', stone: 'Stone', iron: 'Iron', gem: 'Gem' };
